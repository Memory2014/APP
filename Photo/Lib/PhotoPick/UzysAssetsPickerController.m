...
- (void)setupOneMediaTypeSelection
{
    ...
    else
    {
        if(_maximumNumberOfSelectionPhoto == 0)
        {
            ...
            self.labelSelectedMedia.hidden = NO;
            if(_maximumNumberOfSelection > 1)
                self.labelSelectedMedia.text = NSLocalizedString(@"Choose videos", nil); //NSLocalizedStringFromTable(@"Choose videos", @"UzysAssetsPickerController", nil);
            else
                self.labelSelectedMedia.text = NSLocalizedString(@"Choose video", nil); //NSLocalizedStringFromTable(@"Choose video", @"UzysAssetsPickerController", nil);
        }
        else if(_maximumNumberOfSelectionVideo == 0)
        {
            //            self.assetsFilter = [ALAssetsFilter allPhotos];
            ...
            self.labelSelectedMedia.hidden = NO;
            if(_maximumNumberOfSelection >1)
                self.labelSelectedMedia.text = NSLocalizedString(@"Choose photo", nil);  //NSLocalizedStringFromTable(@"Choose photo", @"UzysAssetsPickerController", nil);
            else
                self.labelSelectedMedia.text = NSLocalizedString(@"Choose photo", nil);  //NSLocalizedStringFromTable(@"Choose photo", @"UzysAssetsPickerController", nil);
        }
        else
