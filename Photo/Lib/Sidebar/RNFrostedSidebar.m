...
@property (nonatomic, strong) NSMutableArray *itemLabel;
...
- (instancetype)initWithImages:(NSArray *)images WithLabels:(NSArray *)labels selectedIndices:(NSIndexSet *)selectedIndices borderColors:(NSArray *)colors {
    if (self = [super init]) {
        _isSingleSelect = NO;
        ...
        _itemLabel = [NSMutableArray array];
        _tintColor = [UIColor colorWithWhite:0.98 alpha:0.73];
        _borderWidth = 2;
        _itemBackgroundColor = [UIColor colorWithRed:1 green:1 blue:1 alpha:0.5];
        //_itemBackgroundColor = [UIColor colorWithRed:126 green:211 blue:32 alpha:0.5];
        ...
        [_labels enumerateObjectsUsingBlock:^(NSString *labelString, NSUInteger idx, BOOL *stop){
            
            UILabel *label = [[UILabel alloc]init];
            label.text = labelString;
            label.textColor = [UIColor colorWithRed:0.6588 green:0.6706 blue:0.6824 alpha:1];
            label.font = [UIFont fontWithName:@"PingFangHK-Light" size:15];
            [_contentView addSubview:label];
            [_itemLabel addObject:label];
...
- (void)layoutItems {
    CGFloat leftPadding = (self.width - self.itemSize.width)/2 - 30;
    ...
    [self.itemLabel enumerateObjectsUsingBlock:^(UILabel *label, NSUInteger idx, BOOL *stop){
        CGRect frame = CGRectMake(leftPadding + 80, topPadding*idx + self.itemSize.height*idx + topPadding + 20, 90, 80);
        label.frame = frame;
    }];
