...
- (instancetype)initWithTitle:(NSString *)title body:(NSString *)body image:(UIImage *)image buttonText:(NSString *)buttonText action:(dispatch_block_t)action {
    ...
    if(_image) {
        //CGFloat scale = [UIScreen mainScreen].bounds.size.height/3;
        self.iconHeight = [UIScreen mainScreen].bounds.size.width/2;  //_image.size.height;
        self.iconWidth =  [UIScreen mainScreen].bounds.size.width/2; //_image.size.width;
	}
