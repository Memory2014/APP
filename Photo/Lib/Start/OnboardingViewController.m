...
- (void)initContents:(NSArray *)contents {
...
- (void)viewDidLoad {
    ...
    OnboardingContentViewController *firstPage = [OnboardingContentViewController contentWithTitle:NSLocalizedString(@"Welcome Beautiful Photo", nil)  body:NSLocalizedString(@"This APP will work better when the APP permission to Access Your Photos", nil) image:nil buttonText:@"" action:^{
        //[[[UIAlertView alloc] initWithTitle:nil message:@"Here you" delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil] show];
    }];
    
    OnboardingContentViewController *secondPage = [OnboardingContentViewController contentWithTitle:NSLocalizedString(@"", nil) body:NSLocalizedString(@"Please set the password for private photos. Then you can click  + on the right of app home navigation bar, input the password into the private photos . And you also can input other words to crete new album ", nil) image:[UIImage imageNamed:@"copy"] buttonText:@"" action:^{
        
    }];
    
    OnboardingContentViewController *thirdPage = [OnboardingContentViewController contentWithTitle:NSLocalizedString(@"Now", nil) body:NSLocalizedString(@" Start enjoy the APP", nil) image:nil buttonText:NSLocalizedString(@"Get Started", nil) action:^{
        [self handleOnboardingCompletion];
    }];
    
    
    UIImage *startImage = [UIImage imageNamed:@"firststart.jpg"];

    self.viewControllers = @[firstPage, secondPage, thirdPage];
    // store the passed in view controllers array
    self.backgroundImage = startImage;
    
    [self initContents:@[firstPage, secondPage, thirdPage]];
    ...
    self.titleTextColor = [UIColor yellowColor];
    
    self.titleFontSize = 20;
    self.bodyFontSize = 15;
    self.fontName = @"Optima-BoldItalic";
...
- (void)handleOnboardingCompletion {
    ...
    if (_isSetPassword) {
        UIViewController *homeControl = [mainStoryboard instantiateViewControllerWithIdentifier:@"NavigationHome"];
        ...
        UIViewController *settingControl = [mainStoryboard instantiateViewControllerWithIdentifier:@"SettingControl"];
        settingControl.modalTransitionStyle = UIModalTransitionStyleCrossDissolve;
        [self.navigationController presentViewController:settingControl animated:YES completion:^{}];
    }
