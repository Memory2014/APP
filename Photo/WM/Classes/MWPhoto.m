...
//    
//    if (_requestID == nil) {
//        _requestID = [[NSMutableArray alloc] init];
//    }
    ...
    _assetRequestID = [imageManager requestImageForAsset:asset targetSize:targetSize contentMode:PHImageContentModeAspectFit options:options resultHandler:^(UIImage *result, NSDictionary *info) {
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            //self.caption = result.description;
            
            NSDate *date = asset.creationDate;
            
            //CLLocation *lacation =  asset.location;
            ...
            NSString *create = [NSString stringWithString:NSLocalizedString(@"CreationDate", nil)];
            NSString *pixel = [NSString stringWithString:NSLocalizedString(@"Pixel", nil)];
            
            NSString *favor = asset.favorite ? @"\n♡":@"";
            

//            NSByteCountFormatter *formatter = [[NSByteCountFormatter alloc] init];
//            [imageManager requestImageDataForAsset:asset options:nil resultHandler:^(NSData *imageData, NSString *dataUTI, UIImageOrientation orientation, NSDictionary *info) {
//                long long  byteCount = imageData.length;
//                //convert to Megabytes
//                //imageSize = imageSize/(1024*1024);
//                //NSLog(@"%f",imageSize);
//            }];
            
            //long long byteCount = result.leftCapWidth;
            
            //NSLog(@"%@", [formatter stringFromByteCount:byteCount]);
            
            NSString *cap = [NSString stringWithFormat:@"%@ : %@  %@  %@ : %@ * %@ %@",create, destDateString,@"\n",pixel,
                             @(asset.pixelWidth).stringValue, @(asset.pixelHeight).stringValue,favor];
    ...
    //[_requestID addObject:[NSNumber numberWithUnsignedInteger:_assetRequestID]];
...
- (void) cancelAnyLoadingPHAsset{

//    for (NSNumber *reID in _requestID) {
//        NSUInteger intID = [reID integerValue];
//        if (intID != PHInvalidImageRequestID) {
//            [[PHImageManager defaultManager] cancelImageRequest:(PHImageRequestID)intID];
//            intID = PHInvalidImageRequestID;
//        }
//    }
}
